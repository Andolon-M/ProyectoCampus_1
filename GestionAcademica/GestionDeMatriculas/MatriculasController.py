...
def mostrarCamper(registro):   
    ...
    fila = [
        ...
        registro['matricula'].get('estado', ''),
        registro['matricula'].get('riesgo', ''),
        registro['matricula']["ruta"].get('nombreRuta', ''),
        grupo_camper,
        registro['matricula'].get('fechaInicio', ''),
        registro['matricula'].get('FechaDeFinalizacion', '')
        
    ]
    tabla_datos.append(fila)
            
    # Definir los nombres de las columnas
    headers = ["ID", "DNI", "Nombres", "Apellidos", "Resultado Prueba de Admision", "Estado", "Riesgo", "Ruta Asignada", "Grupo Asignado", "Fecha de inicio", "Fecha de Finalizacion"]
    print(tabulate(tabla_datos, headers=headers, tablefmt="rounded_grid", numalign="left", stralign="left"))
        ...
        while True:
            ...
            if opciones == "":
                ...
                if menuModificar == "1":
                    ...
                    if campoModificar.lower() != "matricula":
                        if campoModificar.lower() in Camper:
                            ...
                            if confirmarDecision():
                                ...
                                print("Regresando...")
                            break
                        elif campoModificar.lower() in Camper["matricula"]:
                            print("El campo",campoModificar," actualmente contiene", Camper["matricula"][campoModificar.lower()])
                            nuevoEstado = input("Ingrese el nuevo valor: ")                                    
                            print("Esta seguro de cambiar ",campoModificar," del Camper", Camper["dni"], "de", Camper["matricula"][campoModificar.lower()],"a", nuevoEstado)
                            if confirmarDecision():
                                Camper["matricula"][campoModificar.lower()]=nuevoEstado.upper()
                                actualizarCamper(Camper["id"], Camper)
                            else:
                                print("Regresando...")
                            break
                        ...
                        print("Si decea modificar la matricula debe ingresar el nombre del campo espesifico...")
                ...
                if confirmarDecision():
                    Camper["matricula"]["estado"]="Expulsado"
                    Camper["matricula"]["ruta"]={}
                    Camper["matricula"]["riesgo"]="N/A"
                    actualizarCamper(Camper["id"], Camper)
